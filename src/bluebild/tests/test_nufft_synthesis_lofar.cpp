...
static auto get_lofar_input_json() -> const nlohmann::json& {
  static nlohmann::json data = []() {
    std::ifstream file(std::string(BLUEBILD_TEST_DATA_DIR) + "/lofar_input.json");
    nlohmann::json j;
    file >> j;
    return j;
  }();
...
static auto get_lofar_nufft_output() -> const nlohmann::json& {
  static nlohmann::json data = []() {
    if constexpr (std::is_same_v<T, float>) {
      std::ifstream file(std::string(BLUEBILD_TEST_DATA_DIR) +
                         "/lofar_nufft_output_single.json");
      nlohmann::json j;
      file >> j;
      return j;
    } else {
...
template <typename T>
class NufftSynthesisLofar : public ::testing::TestWithParam<std::tuple<BluebildProcessingUnit>> {
protected:
  using ValueType = T;

  NufftSynthesisLofar() : ctx_(std::get<0>(GetParam())) {}

  auto test_intensity(BluebildFilter filter, std::string filterString) -> void {
    ...
    bluebild::NufftSynthesis<T> imager(ctx_, tol, nAntenna, nBeam, nIntervals,
                                       1, &filter, nPixel, lmnX.data(),
                                       lmnY.data(), lmnZ.data());
    ...
    for (const auto& itData : data["data"]) {
      ...
      imager.collect(nEig, wl, intervals.data(), 2, s.data(), nBeam, w.data(),
                     nAntenna, xyz.data(), nAntenna, uvw.data(),
                     nAntenna * nAntenna);
    }
  ...
  auto test_sensitivity(BluebildFilter filter, std::string filterString) -> void {
    const auto data = get_lofar_input_json();
    const auto output_data = get_lofar_nufft_output<T>();
    ...
    for (const auto& itData : data["data"]) {
      ...
      imager.collect(nEig, wl, intervals.data(), 2, nullptr, 0, w.data(),
                     nAntenna, xyz.data(), nAntenna, uvw.data(),
                     nAntenna * nAntenna);
    }
...
TEST_P(NufftSynthesisLofarSingle, Intensity_LSQ) { this->test_intensity(BLUEBILD_FILTER_LSQ, "lsq"); }
TEST_P(NufftSynthesisLofarDouble, Intensity_LSQ) { this->test_intensity(BLUEBILD_FILTER_LSQ, "lsq"); }

TEST_P(NufftSynthesisLofarSingle, Intensity_STD) { this->test_intensity(BLUEBILD_FILTER_STD, "std"); }
TEST_P(NufftSynthesisLofarDouble, Intensity_STD) { this->test_intensity(BLUEBILD_FILTER_STD, "std"); }

TEST_P(NufftSynthesisLofarSingle, Intensity_INV) { this->test_intensity(BLUEBILD_FILTER_STD, "inv"); }
TEST_P(NufftSynthesisLofarDouble, Intensity_INV) { this->test_intensity(BLUEBILD_FILTER_STD, "inv"); }
...
static auto param_type_names(
    const ::testing::TestParamInfo<std::tuple<BluebildProcessingUnit>>& info) -> std::string {
...
INSTANTIATE_TEST_SUITE_P(
    Lofar, NufftSynthesisLofarSingle,
    ::testing::Combine(::testing::Values(TEST_PROCESSING_UNITS)),
    param_type_names);

INSTANTIATE_TEST_SUITE_P(
    Lofar, NufftSynthesisLofarDouble,
    ::testing::Combine(::testing::Values(TEST_PROCESSING_UNITS)),
    param_type_names);
