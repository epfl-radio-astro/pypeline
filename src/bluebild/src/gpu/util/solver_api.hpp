...
inline auto convert_jobz(char j){
#ifdef BLUEBILD_MAGMA
  switch (j) {
    case 'N':
    case 'n':
...
inline auto convert_range(char r){
#ifdef BLUEBILD_MAGMA
  switch (r) {
    case 'A':
    case 'a':
...
inline auto convert_uplo(char u){
#ifdef BLUEBILD_MAGMA
  switch (u) {
    case 'L':
    case 'l':
...
auto solve(ContextInternal &ctx, char jobz, char range, char uplo, int n,
  ...
  const float abstol = 2 * slamch_("S", 1);
  ...
  magma_cheevx_gpu(jobzEnum, rangeEnum, uploEnum, n, a, lda, vl, vu, il, iu,
                   abstol, m, wHost.get(), z.get(), ldz, wA.get(), n, wZ.get(), n, &worksize,
                   -1, rwork.get(), iwork.get(), ifail.get(), &info);
  int lwork = static_cast<int>(worksize.x);
  if (lwork < 2 * n)
    lwork = 2 * n;
  auto work = create_buffer<ComplexType<float>>(ctx.allocators().host(), lwork);
  magma_cheevx_gpu(jobzEnum, rangeEnum, uploEnum, n, a, lda, vl, vu, il, iu,
                   abstol, m, wHost.get(), z.get(), ldz, wA.get(), n, wZ.get(), n, work.get(),
                   lwork, rwork.get(), iwork.get(), ifail.get(), &info);
  ...
  int lwork = 0;
  if (cusolverDnCheevdx_bufferSize(ctx.gpu_solver_handle(), jobzEnum, rangeEnum, uploEnum, n, a, lda, vl, vu, il, iu, nullptr,
                                   nullptr, &lwork) != CUSOLVER_STATUS_SUCCESS)
    throw EigensolverError();

  auto workspace = create_buffer<ComplexType<float>>(ctx.allocators().gpu(), lwork);
  auto devInfo = create_buffer<int>(ctx.allocators().gpu(), 2);
  if (cusolverDnCheevdx(ctx.gpu_solver_handle(), jobzEnum, rangeEnum, uploEnum, n, a, lda,
...
auto solve(ContextInternal &ctx, char jobz, char range, char uplo, int n,
                  gpu::ComplexType<double> *a, int lda, double vl, double vu,
                  int il, int iu, int *m, double *w) -> void {
  auto jobzEnum = convert_jobz(jobz);
  auto rangeEnum = convert_range(range);
  auto uploEnum = convert_uplo(uplo);
...
#ifdef BLUEBILD_MAGMA
  gpu::check_status(gpu::stream_synchronize(ctx.gpu_stream()));
  const double abstol = 2 * dlamch_("S", 1);
  ...
  magma_zheevx_gpu(jobzEnum, rangeEnum, uploEnum, n, a, lda, vl, vu, il, iu,
                   abstol, m, wHost.get(), z.get(), ldz, wA.get(), n, wZ.get(), n, &worksize,
                   -1, rwork.get(), iwork.get(), ifail.get(), &info);
  int lwork = static_cast<int>(worksize.x);
  if (lwork < 2 * n)
    lwork = 2 * n;
  auto work = create_buffer<ComplexType<double>>(ctx.allocators().host(), lwork);
  magma_zheevx_gpu(jobzEnum, rangeEnum, uploEnum, n, a, lda, vl, vu, il, iu,
                   abstol, m, wHost.get(), z.get(), ldz, wA.get(), n, wZ.get(), n, work.get(),
                   lwork, rwork.get(), iwork.get(), ifail.get(), &info);
  ...
  auto workspace = create_buffer<ComplexType<double>>(ctx.allocators().gpu(), lwork);
  auto devInfo = create_buffer<int>(ctx.allocators().gpu(), 2);
...
auto solve(ContextInternal &ctx, char jobz, char range, char uplo, int n,
...
#ifdef BLUEBILD_MAGMA
  auto aHost = create_buffer<ComplexType<float>>(ctx.allocators().pinned(), n*n);
  auto bHost = create_buffer<ComplexType<float>>(ctx.allocators().pinned(), n*n);
  auto zHost = create_buffer<ComplexType<float>>(ctx.allocators().pinned(), n*n);
  gpu::check_status(
  ...
  int info = 0;
  gpu::ComplexType<float> worksize;
  magma_chegvx(1, jobzEnum, rangeEnum, uploEnum, n, aHost.get(), n, bHost.get(), n,
               vl, vu, il, iu, abstol, m, wHost.get(), zHost.get(), ldz,
               &worksize, -1, rwork.get(),
  ...
  int lwork = 0;
  if (cusolverDnChegvdx_bufferSize(ctx.gpu_solver_handle(), CUSOLVER_EIG_TYPE_1, jobzEnum,
                                   rangeEnum, uploEnum, n, a, lda, b, ldb, vl,
                                   vu, il, iu, nullptr, nullptr,
                                   &lwork) != CUSOLVER_STATUS_SUCCESS)
    throw EigensolverError();

  auto workspace = create_buffer<ComplexType<float>>(ctx.allocators().gpu(), lwork);
  auto devInfo = create_buffer<int>(ctx.allocators().gpu(), 2);
  if (cusolverDnChegvdx(ctx.gpu_solver_handle(), CUSOLVER_EIG_TYPE_1, jobzEnum, rangeEnum,
                        uploEnum, n, a, lda, b, ldb, vl, vu, il, iu, m, w,
                        workspace.get(), lwork, devInfo.get()) != CUSOLVER_STATUS_SUCCESS)
    throw EigensolverError();
  gpu::check_status(gpu::stream_synchronize(ctx.gpu_stream()));
#endif
}
...
auto solve(ContextInternal &ctx, char jobz, char range, char uplo, int n,
...
#ifdef BLUEBILD_MAGMA
  auto aHost = create_buffer<ComplexType<double>>(ctx.allocators().pinned(), n*n);
  auto bHost = create_buffer<ComplexType<double>>(ctx.allocators().pinned(), n*n);
  auto zHost = create_buffer<ComplexType<double>>(ctx.allocators().pinned(), n*n);
  gpu::check_status(
  ...
  const double abstol = 2 * slamch_("S", 1);
  ...
  int info = 0;
  gpu::ComplexType<double> worksize;
  magma_zhegvx(1, jobzEnum, rangeEnum, uploEnum, n, aHost.get(), n, bHost.get(), n,
               vl, vu, il, iu, abstol, m, wHost.get(), zHost.get(), ldz,
               &worksize, -1, rwork.get(),
               iwork.get(), ifail.get(), &info);
  int lwork = static_cast<int>(worksize.x);
...
#else
  int lwork = 0;
  if (cusolverDnZhegvdx_bufferSize(ctx.gpu_solver_handle(), CUSOLVER_EIG_TYPE_1, jobzEnum,
                                   rangeEnum, uploEnum, n, a, lda, b, ldb, vl,
                                   vu, il, iu, nullptr, nullptr,
                                   &lwork) != CUSOLVER_STATUS_SUCCESS)
    throw EigensolverError();
  ...
  auto workspace = create_buffer<ComplexType<double>>(ctx.allocators().gpu(), lwork);
  auto devInfo = create_buffer<int>(ctx.allocators().gpu(), 2);
  if (cusolverDnZhegvdx(ctx.gpu_solver_handle(), CUSOLVER_EIG_TYPE_1, jobzEnum, rangeEnum,
                        uploEnum, n, a, lda, b, ldb, vl, vu, il, iu, m, w,
                        workspace.get(), lwork, devInfo.get()) != CUSOLVER_STATUS_SUCCESS)
    throw EigensolverError();
  gpu::check_status(gpu::stream_synchronize(ctx.gpu_stream()));
