...
template <typename T>
static __global__ void gram_kernel(std::size_t n, const T *__restrict__ x,
                                   const T *__restrict__ y,
                                   const T *__restrict__ z, T wl,
                                   gpu::ComplexType<T> *g, std::size_t ldg) {
  for (std::size_t j = threadIdx.y + blockIdx.y * blockDim.y; j < n;
...
auto gram_gpu(gpu::StreamType stream, std::size_t n, const T *x, const T *y,
  ...
  auto numBlocks =
      std::min<std::size_t>(maxBlocks, (n + block.x - 1) / block.x);
...
template auto gram_gpu<float>(gpu::StreamType stream, std::size_t n,
                              const float *x, const float *y, const float *z,
                              float wl, gpu::ComplexType<float> *g,
                              std::size_t ldg) -> void;

template auto gram_gpu<double>(gpu::StreamType stream, std::size_t n,
                               const double *x, const double *y,
                               const double *z, double wl,
                               gpu::ComplexType<double> *g, std::size_t ldg)
    -> void;
} // namespace bluebild
