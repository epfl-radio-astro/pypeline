    ...
    stages {
        ...
        stage('Seff') {
            ...
            steps {
                ...
                script {
                    JOBID = sh (
                        script: "sbatch --wait --parsable --partition build --time 00-00:15:00 --qos free --gres gpu:1 --mem 40G --cpus-per-task 1 -o ${env.TEST_DIR}/slurm-%j.out ./jenkins/slurm_generic_synthesizer.sh",
                        returnStdout: true
                    ).trim()
