...
for t in ProgressBar(time[::time_slice]):
    ...
    D, V, c_idx = I_dp(S, G)
    c_idx = np.asarray(range(N_level))
    print(c_idx, c_idx.shape)
...
print(lsq_image.shape, sqrt_image.shape )
...
print("test eig 1:", lsq_image.shape)
...
print("test eig 2:", I_lsq_eq.data.shape)
t2 = tt.time()
