...
import pypeline.phased_array.dask_ms as dask_ms
...
daskms = dask_ms.LofarMeasurementSet(ms_file, N_station) # stations 1 - N_station 
...
FoV = np.deg2rad(5)
channel_id = 0
frequency = daskms.channels["FREQUENCY"][channel_id]
wl = constants.speed_of_light / frequency.to_value(u.Hz)
sky_model = source.from_tgss_catalog(daskms.field_center, FoV, N_src=4)
obs_start, obs_end = daskms.time[[0, -1]]
...
_, _, px_colat, px_lon = grid.equal_angle(
    N=daskms.instrument.nyquist_rate(wl), direction=daskms.field_center.cartesian.xyz.value, FoV=FoV
)

print(daskms.instrument.nyquist_rate(wl), "Nstation {0}, px_col {1}, px_lon {2}".format(N_station, px_colat.shape, px_lon.shape))
...
for t, f, S in ProgressBar(
    ...
    XYZ = daskms.instrument(t)
    W = daskms.beamformer(XYZ, wl)
    G = gram(XYZ, W, wl)
    S, _ = dask_ms.filter_data(S, W)
...
for t, f, S in ProgressBar(
        daskms.visibilities(channel_id=[channel_id], time_id=slice(0, 10, 1), column="DATA")
):
    wl = constants.speed_of_light / f.to_value(u.Hz)
    XYZ = daskms.instrument(t)
    W = daskms.beamformer(XYZ, wl)
    S, W = dask_ms.filter_data(S, W)
...
# dirichlet_kernel = SphericalDirichlet(N=daskms.instrument.nyquist_rate(wl), approx=True)
# nside = (daskms.instrument.nyquist_rate(wl) + 1) / 3
...
# f_interp = f_interp / (daskms.instrument.nyquist_rate(wl) + 1)
