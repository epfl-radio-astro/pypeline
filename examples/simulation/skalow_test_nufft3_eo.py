...
#source = coord.SkyCoord(ra=15.0*u.degree, dec=-45*u.degree, frame='icrs')
source = coord.SkyCoord(ra=193.0*u.degree, dec=-67*u.degree, frame='icrs')
...
#for use_ms in False,:
    ...
    for t, f, S, uvw in ms.visibilities(channel_id=[channel_id], time_id=slice(0, None, None), column="DATA", return_UVW=True):
        ...
        lst = atime.Time(t, scale='utc', location=low_location).sidereal_time('mean')
        ...
        for i in range(0, NANT):
            for j in range(0, NANT):
                tmp = rascil_crd.xyz_at_latitude(bsl__enu[i,j,:], ACLAT.rad)
                bsl__uvw[i,j,:] = rascil_crd.xyz_to_uvw(tmp, h0.rad, source.dec.rad)
        print(f"bsl__uvw BB\n {bsl__uvw}")
