...
do3D         = False
doPlan       = False
rascil_fixed = False #True to use MS file generated with RASCIL containing crd bug fix
...
if read_coords_from_ms:
    ...
    cl_WCS = ifits.wcs("/scratch/izar/krishna/MWA/WSClean/1133149192-187-188_Sun_10s_cal1024_Pixels_4_5_channels-image.fits")
...
for use_ms in True, False:
    for use_raw_vis in True, False:
        for do3D in True, :
            ...
            if rascil_fixed:
                outfilename = 'spk_bugfix'
            else:
                outfilename = 'spk_test'
            outfilename += '_mwa_nufft_' + ('msUVW_' if uvw_from_ms else '') + ('rawVis_' if use_raw_vis else 'BBVis')
            ...
            for t, f, S, uvw in ms.visibilities(channel_id=[channel_id], time_id=slice(0, None, None), column="DATA", return_UVW=True):
                ...
                UVW_baselines.append(baseline_rescaling * UVW_baselines_t) # one 2 pi factor
            ...
            UVW_baselines = 2 * np.pi * UVW_baselines.T.reshape(3, -1) / wl # two 2 pi factor
